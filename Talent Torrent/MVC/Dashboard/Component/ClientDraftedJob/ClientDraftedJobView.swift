...
protocol ClientDraftedJobViewDelegate {
    func draftedJobInfo(totalNumber: String)
}

class ClientDraftedJobView: UIView {

    let identifire = "ClientDraftedJobView"
    var delegate: ClientDraftedJobViewDelegate?
    ...
    override func awakeFromNib() {
        ...
        print("------------")
        trailingURL = "client/jobs/drafted"
        ...
        apiRequest.sendAPIRequestBackData(trailingURL: trailingURL, method: .get, parameters: nil, headers: headers) { json, error in
            ...
            self.delegate?.draftedJobInfo(totalNumber: totalJob)
        }
...
extension ClientDraftedJobView: UICollectionViewDataSource, UICollectionViewDelegate {
...
extension ClientDraftedJobView: UICollectionViewDelegateFlowLayout {
