...
protocol ClientActiveJobDelegate {
    func activeJobInfo(totalNumber: String)
}

class ClientActiveJobView: UIView {

    let identifire = "ClientActiveJobView"
    var delegate: ClientActiveJobDelegate?
    ...
    override func awakeFromNib() {
        ...
        trailingURL = "client/jobs/active"
        ...
        apiRequest.sendAPIRequestBackData(trailingURL: trailingURL, method: .get, parameters: nil, headers: headers) { json, error in
            ...
            self.delegate?.activeJobInfo(totalNumber: totalJob)
        }
...
extension ClientActiveJobView: UICollectionViewDataSource, UICollectionViewDelegate {
...
extension ClientActiveJobView: UICollectionViewDelegateFlowLayout {
