...
protocol ClientSubmittedJobViewDelegate {
    func awardedJobInfo(totalNumber: String)
}

class ClientSubmittedJobView: UIView {

    let identifire = "ClientSubmittedJobView"
    var delegate: ClientSubmittedJobViewDelegate?
    ...
    @IBOutlet weak var activeJobCollectionView: UICollectionView!
...
//        EmptyJobShowLabel.text = "No Job Found"
        ...
        activeJobCollectionView.delegate = self
        activeJobCollectionView.dataSource = self
        
        let nib = UINib(nibName: cellIdentity, bundle: nil)
        activeJobCollectionView.register(nib, forCellWithReuseIdentifier: cellIdentity)
        print("------------")
        trailingURL = "client/jobs/submitted"
        ...
        apiRequest.sendAPIRequestBackData(trailingURL: trailingURL, method: .get, parameters: nil, headers: headers) { json, error in
            ...
            self.activeJobCollectionView.reloadData()
            ...
            self.delegate?.awardedJobInfo(totalNumber: totalJob)
        }
...
extension ClientSubmittedJobView: UICollectionViewDataSource, UICollectionViewDelegate {
...
extension ClientSubmittedJobView: UICollectionViewDelegateFlowLayout {
