...
protocol ExpertOngoingJobViewDelegate {
    func expertOngoingJobInfo(totalNumber: String)
}

class ExpertOngoingJobView: UIView {

    let identifire = "ExpertOngoingJobView"
    var delegate: ExpertOngoingJobViewDelegate?
    ...
    override func awakeFromNib() {
        ...
        trailingURL = "expert/jobs/ongoing"
        ...
        apiRequest.sendAPIRequestBackData(trailingURL: trailingURL, method: .get, parameters: nil, headers: headers) { json, error in
            ...
            self.delegate?.expertOngoingJobInfo(totalNumber: totalJob)
        }
...
extension ExpertOngoingJobView: UICollectionViewDataSource, UICollectionViewDelegate {
...
extension ExpertOngoingJobView: UICollectionViewDelegateFlowLayout {
